...
Все запросы уходят ТОЛЬКО на указанный тобой endpoint. Ничего не
...
Опционально, для прямого сравнения с официальным доступом:
...
def main():
    ...
    proxy_url = ask("Введи PROXY URL https://твой-прокси.com/v1")
    proxy_key = ask("Введи PROXY KEY")
    model = ask("Введи PROXY MODEL")

    if not proxy_url or not proxy_key:
        print("\nНужно указать и PROXY URL, и PROXY KEY. Запусти скрипт заново.")
        input("\nНажми Enter, чтобы закрыть окно...")
        sys.exit(1)

    print()
    compare = ask("Сравнить с официальным доступом? (y/n)", default="n")
    official_env_url = official_env_key = None
    if compare.strip().lower() in ("y", "yes", "да", "д"):
        official_env_url = ask("Введи OFFICIAL URL")
        official_env_key = ask("Введи OFFICIAL KEY")
        if official_env_url:
            os.environ["OFFICIAL URL"] = official_env_url
        if official_env_key:
            os.environ["OFFICIAL KEY"] = official_env_key
    print()
    ...
    official_url = os.environ.get("OFFICIAL URL")
    official_key = os.environ.get("OFFICIAL KEY")
