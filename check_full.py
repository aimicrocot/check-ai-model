...
УНИВЕРСАЛЬНЫЙ ЗАПУСК — этот файл сам определяет, как его запустили:

  1) Через батник / cmd (заданы переменные окружения или аргументы) —
     работает без вопросов, без паузы в конце:
     ...
     или так:
       python check_full.py https://твой-прокси.com/v1 твой-ключ deepseek-v4-pro

     Опционально, для сравнения с официальным доступом:
       set OFFICIAL_URL=https://api.deepseek.com/v1
       set OFFICIAL_KEY=твой-официальный-ключ

  2) Двойным кликом мышью (если Python добавлен в PATH и ассоциирован
     с .py) — параметры запросит интерактивно и в конце подождёт Enter,
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if e.code == 429 and attempt < max_retries:
                ...
                if wait > 60:
                    msg = (f"Сервер прислал Retry-After = {wait:.0f}s (это {wait/3600:.1f} ч) — "
                    ...
                    if anomalies is not None:
                        anomalies.append(f"[{context}] {msg}" if context else msg)
                    try:
...
def part2_degradation(base_url, api_key, model, findings, anomalies, label="Прокси"):
    print("\n" + "#" * 60)
    ...
    r = call_chat(base_url, api_key, model, SELF_ID_PROMPT, max_tokens=150,
                  anomalies=anomalies, context=f"{label} / Самоидентификация")
    if "error" in r:
        print(f"  ❌ {r['error']}")
    else:
        ...
        if returned_model:
            ...
            if returned_model != model:
                msg = f"Запрошена '{model}', сервер вернул '{returned_model}'."
                print(f"  ⚠️ {msg}")
                anomalies.append(f"[{label} / Самоидентификация] {msg}")

    for title, prompt in DEGRADATION_TASKS:
        print(f"\n--- {title} ---")
        ctx = f"{label} / {title}"
        time.sleep(2)  # небольшая пауза, чтобы не провоцировать rate limit
        r1 = call_chat(base_url, api_key, model, prompt, anomalies=anomalies, context=ctx)
        if "error" in r1:
        ...
        time.sleep(2)
        r2 = call_chat(base_url, api_key, model, prompt, anomalies=anomalies, context=ctx)
        if "error" in r2:
...
def main():
    ...
    # Ничего не передали ни через env, ни через argv -> значит, скорее
    # всего, файл запустили двойным кликом. Переходим в интерактивный режим.
    interactive = not (proxy_url and proxy_key)

    if interactive:
        print("=" * 60)
        print("  Проверка OpenAI-совместимого прокси")
        print("=" * 60)
        print()
        proxy_url = proxy_url or ask("Введи PROXY URL https://твой-прокси.com/v1")
        proxy_key = proxy_key or ask("Введи PROXY KEY")
        model = model or ask("Введи PROXY MODEL", default="gpt-4o")
    ...
    if interactive:
        print()
        compare = ask("Сравнить с официальным доступом? (y/n)", default="n")
        official_url = official_key = None
        if compare.strip().lower() in ("y", "yes", "да", "д"):
            official_url = ask("Введи OFFICIAL URL")
            official_key = ask("Введи OFFICIAL KEY")
        print()
    else:
    ...
    try:
        part2_degradation(proxy_url, proxy_key, model, findings, anomalies, label="Прокси")
    except Exception as e:
        print(f"\n(Часть 2 прервана: {type(e).__name__}: {e})")

    if official_url and official_key:
        try:
            part2_degradation(official_url, official_key, model, findings, anomalies, label="Официальный доступ")
        except Exception as e:
    ...
    if anomalies:
        print(f"\nПодозрительные моменты, обнаруженные в ходе проверки ({len(anomalies)}):")
        for i, a in enumerate(anomalies, 1):
            print(f"  {i}. {a}")
    else:
        print(
            "\nПодозрительных моментов (нестабильность при temperature=0, "
            "аномальный Retry-After и т.п.) не обнаружено."
        )
    print(
        "\nНапоминание:\n"
        ...
        "  - Это эвристика, не строгое доказательство. Для однозначного\n"
        "    вывода лучше всего сравнение с официальным прямым доступом\n"
        "    (OFFICIAL_URL / OFFICIAL_KEY).\n"
    )
